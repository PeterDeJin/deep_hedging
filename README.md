...
| `deep_hedging_with_cost_model _final.pdf` | 專案完整報告（CVaR／Entropic、交易成本模型、AAPL／AMZN 實證）|
| `Deep Hedging with Transaction Cost Model.docx` | 報告（Word 版）|
| `*.png`（40+ 張）| 各情境結果圖：PnL、CVaR／Entropic 訓練曲線、delta hedging 比較、無/美國/台灣成本、AAPL／AMZN、避險權重等 |
